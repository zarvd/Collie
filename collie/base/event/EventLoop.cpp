...
void EventLoop::enableEventRead(unsigned & events) const noexcept {
    poller->enableRead(events);
}

void EventLoop::disableEventRead(unsigned & events) const noexcept {
    poller->disableRead(events);
}

void EventLoop::enableEventWrite(unsigned & events) const noexcept {
    poller->enableWrite(events);
}

void EventLoop::disableEventWrite(unsigned & events) const noexcept {
    poller->disableWrite(events);
}

bool EventLoop::isEventRead(const unsigned & events) const noexcept {
    return poller->isRead(events);
}

bool EventLoop::isEventWrite(const unsigned & events) const noexcept {
    return poller->isWrite(events);
}

bool EventLoop::isEventError(const unsigned & events) const noexcept {
    return poller->isError(events);
}

bool EventLoop::isEventClose(const unsigned & events) const noexcept {
