...
void
testSocket(const unsigned port) {
    Socket socket(port);
    socket.listen();
...
void
testEPoller(const unsigned) {}

void
testEventLoop(const unsigned port) {
    std::shared_ptr<EventLoop> eventLoop(new EventLoop);
    Socket socket(port);
    socket.listen();
    std::shared_ptr<Channel> acceptChannel(
        new Channel(eventLoop, socket.getFd()));
...
void
testTcpServer(const unsigned port) {
    TcpServer tcp(port);

    ConnectCallback acceptCB = [&tcp](const int connFd) {
        std::shared_ptr<Channel> connChannel(
            new Channel(tcp.getEventLoop(), connFd));
